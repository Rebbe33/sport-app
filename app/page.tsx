...
useEffect(() => {
  ...
  const allTodayDone = todaySessions.length > 0 && todaySessions.every(s => s.status === 'done')
  ...
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 4 }}>
    ...
    <div style={{ display: 'flex', gap: 4 }}>
      ...
      {showBilanReminder && (
        <Link href="/bilan">
          <div style={{ margin: '12px 20px 0', background: 'var(--accent-light)', border: '1px solid var(--accent)', borderRadius: 14, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 24 }}>📊</span>
            <div style={{ flex: 1 }}>
              <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14, color: 'var(--text)' }}>Bilan des 2 dernières semaines</p>
              <p style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 2 }}>C'est le moment de faire le point !</p>
            </div>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      {!done ? (
                        <Link href={`/session/${s.id}`} style={{ flex: 2 }}>
                          <button style={{ width: '100%', padding: '14px 20px', background: meta.color, border: 'none', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, color: 'white', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {weekDays.map(day => {
              ...
              // Lien : plusieurs séances → page jour, une seule → session directe
              const dayLink = hasMultiple
                ? `/planning/${format(day, 'yyyy-MM-dd')}`
                : s && !allDone ? `/session/${s.id}` : null
              ...
              return (
                <div key={day.toISOString()} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
                  ...
                  {dayLink ? (
                    <Link href={dayLink}>
                      <div style={{ width: 34, height: 34, borderRadius: 10, background: meta!.color + '30', border: today ? '2px solid var(--accent)' : '1px solid transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                    ...
                    <div style={{ width: 34, height: 34, borderRadius: 10, background: s ? meta!.color : 'var(--surface2)', border: today ? '2px solid var(--accent)' : '1px solid transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
